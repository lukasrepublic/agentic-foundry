...
2. **The gate workflows** (templates in this repo's own `.github/workflows/` — copy or
   adapt): `spec-link` (a code-change PR must name the spec that authorized it; docs-only
   diffs are not-applicable) and `security-path` (a diff touching auth/secrets/dependency
   surfaces requires a posted security-review verdict). Both are **always-reporting**: they
