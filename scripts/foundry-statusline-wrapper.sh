#!/usr/bin/env bash
# foundry-statusline-wrapper.sh — version-agnostic, fail-open self-resolving statusLine wrapper
# (feat-foundry-init-statusline-wrapper, AC-SLW-1).
#
# /foundry:init installs this into an adopter repo at .claude/hooks/foundry-statusline.sh and wires the
# adopter's statusLine.command to "$CLAUDE_PROJECT_DIR/.claude/hooks/foundry-statusline.sh" — the
# EXPANDABLE placeholder. (The plugin-root hook path-placeholder is HOOK-scoped and does NOT expand in a
# statusLine command, so this wrapper references it NOWHERE.) This thin resolver discovers the newest
# INSTALLED foundry renderer in the plugin cache and execs it, passing the status payload through stdin.
#
...
# Resolve the newest installed renderer by the VERSION path-segment. The `[ -f ]` guard makes a no-match
# glob (which bash leaves as the literal pattern) yield nothing — no nullglob dependency.
selected="$(
  for cand in "${HOME}/.claude/plugins/cache/"*/foundry/*/scripts/foundry-statusline.sh; do
    [ -f "$cand" ] || continue
...
# exec the selected renderer: stdin (the status payload) passes through; forward "$@".
exec bash "$selected" "$@"
