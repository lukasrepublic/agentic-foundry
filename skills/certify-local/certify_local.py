...
A THIN procedure over native primitives — no verdict engine, no custom evidence format:
  ...
  2. Resolve the active stack profile's boot recipe — `app_exercise_binding.boot` — via the SAME
     resolution `scripts/foundry-verify.py` uses (`scripts/foundry-stack-profile.py`'s
     `read_lock`/`resolve_lock`, imported read-only, never redefined here).
...
these is a PRE-DISPATCH refusal, message-prefixed `REFUSED_PREFIX` ("nothing dispatched"):
  ...
  * no active `.foundry/stack-profile.lock`, or the resolved profile carries no
    `app_exercise_binding` (a `profile_kind: infra` profile, or an infra-only lock) ("no boot recipe").
...
def certify(release_id, *, project_dir=None, plugin_root=None, root=None,
    ...
    repo_root = resolve_target_repo(release, pd, release_mod)   # raises _refuse on a repo split
    ...
    profile, binding = resolve_boot_recipe(pd, pr, root=root)   # raises _refuse: "no boot recipe: …"
    ...
    try:
        boot_proc = subprocess.Popen(binding["boot"], shell=True, cwd=repo_root,
